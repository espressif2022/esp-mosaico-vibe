...
static underwater_view_atlases_t atlases;
...
static esp_err_t load_scene_background(uint8_t scene)
{
    ...
    atlases.aurora=atlases.ocean=atlases.sunrise=atlases.rainforest=atlases.panorama=next;
    loaded_scene=scene;
    return ESP_OK;
}
...
static esp_err_t before_display(void)
{
    ...
    ESP_RETURN_ON_ERROR(load_scene_volumes(UNDERWATER_SCENE_OCEAN),"underwater_360","load ocean volumes");
...
static void on_render(void){underwater_view_render(&world,&atlases);}
